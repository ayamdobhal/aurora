...
Auto lyrics launches all three providers and compares their bounded results: word timing, line timing, plain text, then instrumental. Ties retain AMLL/Spotify/lrclib priority. Spotify lyrics use the authenticated native request builder when available, with Cosmos retained for older clients. Explicit provider selection calls only that provider. Positive results have a ten-minute cache; missing results expire sooner, and failures are not cached. Obsolete results cannot replace a newer view. Requests still shared by other views are not cancelled merely because one view closes.
